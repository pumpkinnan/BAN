...
def non_local_block(input_x, input_y, scope):
    y_channel = input_y.get_shape()[-1].value
    x_channel = input_x.get_shape()[-1].value

    g = conv1d(input_y, y_channel, 1, padding='VALID', activation_fn=None, scope='g_%s' % scope)
    phi = conv1d(input_x, x_channel, 1, padding='VALID', activation_fn=None, scope='phi_%s' % scope)
    theta = conv1d(input_x, x_channel, 1, padding='VALID', activation_fn=None, scope='theta_%s' % scope)

    theta_x = tf.transpose(theta, [0, 2, 1])
    f = tf.matmul(phi, theta_x)
    f_softmax = tf.nn.softmax(f)
    y = tf.matmul(f_softmax, g)
    output_z = tf.concat([input_y, y], 2)
    return output_z


'''
def non_local_block_1(input_x, input_y):
    n = input_y.get_shape()[-1].value
    m = input_x.get_shape()[-1].value
    g = conv1d(input_y, n, 1,  padding='VALID', activation_fn=None, scope='g_1')
    phi = conv1d(input_x, m, 1, padding='VALID', activation_fn=None, scope='phi_1')
    theta = conv1d(input_x, m, 1, padding='VALID', activation_fn=None, scope='theta_1')

    theta_x = tf.transpose(theta, [0, 2, 1])
    f = tf.matmul(phi, theta_x)
    f_softmax = tf.nn.softmax(f)
    y = tf.matmul(f_softmax, g)
    output = tf.concat([input_y, y], 2)
    return output, f_softmax

def non_local_block_back(input_x, input_y, scope):
    n = input_y.get_shape()[-1].value
    m = input_x.get_shape()[-1].value
    g = conv1d(input_y, n, 1,  padding='VALID', activation_fn=None, scope='g_%s'%(scope))
    phi = conv1d(input_x, m, 1, padding='VALID', activation_fn=None, scope='phi_%s'%(scope))
    theta = conv1d(input_x, m, 1, padding='VALID', activation_fn=None, scope='theta_%s'%(scope))
...
def non_local_block_back_1(input_x, input_y, scope):
    n = input_y.get_shape()[-1].value
    m = input_x.get_shape()[-1].value
    g = conv1d(input_y, n, 1,  padding='VALID', activation_fn=None, scope='g_%s'%(scope))
    phi = conv1d(input_x, m, 1, padding='VALID', activation_fn=None, scope='phi_%s'%(scope))
    ...
    return output, f_softmax
'''
def self_attention(input, scope):
    n = input.get_shape()[-1].value
    g = conv1d(input, n, 1,  padding='VALID', activation_fn=None, scope='g_%s'%(scope))
    phi = conv1d(input, n, 1, padding='VALID', activation_fn=None, scope='phi_%s'%(scope))
    theta = conv1d(input, n, 1, padding='VALID', activation_fn=None, scope='theta_%s'%(scope))
    theta_x = tf.transpose(theta, [0, 2, 1])
    f = tf.matmul(phi, theta_x)
    f_softmax = tf.nn.softmax(f)
    y = tf.matmul(f_softmax, g)
    output = tf.concat([input, y], 2)
    return output
'''
def self_attention_back(input, scope):
    n = input.get_shape()[-1].value
    g = conv1d(input, n, 1,  padding='VALID', activation_fn=None, scope='g_%s'%(scope))
    phi = conv1d(input, n, 1, padding='VALID', activation_fn=None, scope='phi_%s'%(scope))
    theta = conv1d(input, n, 1, padding='VALID', activation_fn=None, scope='theta_%s'%(scope))
