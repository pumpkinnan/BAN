...
def BlockMerging(volume, volume_seg, pts, grouplabel, groupseg, gap=1e-3):
    ...
    #print(np.max(maxoverlapgroupcounts))
...
def eval_3d_perclass(tp, fp, npos):
    ...
    tp = np.cumsum(tp) #?xinlongw: wrong order
    fp = np.cumsum(fp)
    rec = tp / npos
    prec = tp / (fp+tp)
...
color_map = json.load(open('/home/wuguangnan/study/ASIS-master/models/ASIS/part_color_mapping.json', 'r'))
